...
import { useRouter } from "next/navigation"; // <<< PASSO 1: IMPORTE O ROUTER

// Suas outras importações de componentes UI
import {
  Sheet,
  SheetClose,
  SheetContent,
...
const BookingItem = ({ booking }: BookingItemProps) => {
  const router = useRouter(); // <<< PASSO 2: INSTANCIE O ROUTER
  ...
  const handleCancelBooking = async () => {
    try {
      ...
      // <<< PASSO 3: CHAME O REFRESH APÓS O SUCESSO DO CANCELAMENTO >>>
      // Isso irá recarregar os dados da página Home, atualizando a lista de agendamentos.
      router.refresh(); 
